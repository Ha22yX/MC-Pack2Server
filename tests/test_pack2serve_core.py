...
class Pack2ServeCoreTests(unittest.TestCase):
    ...
    def test_server_builder_isolates_curseforge_lan_operator_mods(self) -> None:
        from pack2serve.downloader import ResolvedCurseForgeArtifact

        class Provider:
            name = "operator-provider"

            def __init__(self, url_by_file_id: dict[int, str], name_by_file_id: dict[int, str]):
                self.url_by_file_id = url_by_file_id
                self.name_by_file_id = name_by_file_id
            ...
            def resolve(self, context):
                return ResolvedCurseForgeArtifact(
                    ...
                    project_id=context.project_id,
                    file_id=context.file_id,
                    file_name=self.name_by_file_id[context.file_id],
                    download_url=self.url_by_file_id[context.file_id],
                )

        with tempfile.TemporaryDirectory() as temp:
            tmp_path = Path(temp)
            lan_source = tmp_path / "lan-operators-1.20.1-1.0.1.jar"
            auto_source = tmp_path / "auto_mod-1.0-fabric-1.20.1.jar"
            lan_source.write_bytes(b"lan-operators")
            auto_source.write_bytes(b"auto-operator")
            pack = tmp_path / "sample.zip"
            write_zip(
                pack,
                {
                    "manifest.json": json.dumps(
                        {
                            ...
                            "files": [
                                {"projectID": 614516, "fileID": 4670440, "required": True},
                                {"projectID": 1308490, "fileID": 6778301, "required": True},
                            ],
                            "overrides": "overrides",
                        }
                    ),
                    "modlist.html": (
                        '<ul>'
                        '<li><a href="https://www.curseforge.com/minecraft/mc-mods/lan-operators">'
                        "LAN Operators</a></li>"
                        '<li><a href="https://www.curseforge.com/minecraft/mc-mods/auto-operator">'
                        "Auto operator (LAN supported)</a></li>"
                        "</ul>"
                    ),
                },
            )
            ...
            report = ServerBuilder(
                ...
                curseforge_providers=[
                    Provider(
                        {
                            4670440: lan_source.as_uri(),
                            6778301: auto_source.as_uri(),
                        },
                        {
                            4670440: lan_source.name,
                            6778301: auto_source.name,
                        },
                    )
                ],
            ).build(pack, target)

            self.assertFalse((target / "mods/lan-operators-1.20.1-1.0.1.jar").exists())
            self.assertFalse((target / "mods/auto_mod-1.0-fabric-1.20.1.jar").exists())
            self.assertEqual(
                (target / "_client-overrides/mods/lan-operators-1.20.1-1.0.1.jar").read_bytes(),
                b"lan-operators",
            )
            self.assertEqual(
                (target / "_client-overrides/mods/auto_mod-1.0-fabric-1.20.1.jar").read_bytes(),
                b"auto-operator",
            )
            self.assertEqual(
                [item.classification for item in report.copied_overrides[-2:]],
                ["client-remote-isolated", "client-remote-isolated"],
            )
